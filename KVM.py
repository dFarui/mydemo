    ...
    def inspect_disk_info_for_down(self,instance_name):
        ...
        for device in filter(
            ...
            try:
                ...
                disk_size = [0,0,0]
                pass
            size = DiskSize(total=disk_size[0]/(1024*1024),allocation=disk_size[1]/(1024*1024),physical=disk_size[2]/(1024*1024))
            yield (disk,size)
...
def output():
    ...
    for instance in not_running_instances:
        ...
        nicList = []
        nicdict = {'nic_name':'','mac':'','ip':'','net_send_read':'','net_receive_write':'','net_send_request':'','net_receive_reques':''}
    ...
    for instance in instances:
        ...
        dict['disks']=diskList
        list.append(dict)
    f=open("/opt/server/cml/cml_agent/var/vm_monitor.json","w")
    try:
